...
«عالم حكيم» تطبيق تعليمي ترفيهي فاخر للأطفال السعوديين، **المرحلة الابتدائية حصريًّا (الصفوف ١-٦)**. الفكرة الجوهرية:
...
- `stages` (المراحل)، `grades` (الصفوف ١-٦)، `subjects` (المواد)، `lessons` (الدروس: id, title, content_text, status, subject_id, grade_id).
...
### بيانات موجودة
- مرحلة ابتدائية + صفوف ١-٦ + مادة الرياضيات + درس «مقدمة في الضرب» (status=processed).
...
- `tutor-chat`: **عقل المعلّم الحواري الجديد** (مرفوعة، تحتاج نشرًا واختبارًا). تستقبل سجلّ المحادثة + ردّ الطفل، تحلّل الفهم، تقرّر مسار حكيم دون صح/خطأ، تُرجع: reply, understanding, concept, lessonComplete, suggestChips.
...
1. **المعلّم الحواري**: نشر `tutor-chat` واختبارها، ثم إعادة بناء `lesson.tsx` كشاشة حوار صوتية.
